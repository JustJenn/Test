...
package gs
{
	...
	public class TweenMax extends TweenFilterLite
	{
		...
		{ //vars takes the same special parameters as to() and from() calls, and ALSO "delayIncrement", "onCompleteAll", "onCompleteAllParams", and "onCompleteAllScope"
			if ($targets.length == 0)
			{
				return [];
			}
		...
		{ //Note: make sure each tween object has a "time" parameter (duration in seconds).
...
//---- PROXY FUNCTIONS ------------------------------------------------------------------------------------------------------------
		...
		{ //Only for orientToBezier tweens. Separated it for speed.
			bezierProxy($o);
			var future:Object={};
